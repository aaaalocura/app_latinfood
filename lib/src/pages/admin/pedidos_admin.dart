...
body: FutureBuilder<List<Sale>>(
  ...
  builder: (context, snapshot) {
    if (snapshot.connectionState == ConnectionState.waiting) {
      ...
      final pendingSales = sales?.where((sale) => sale.status == 'PENDING').toList();

      if (pendingSales == null || pendingSales.isEmpty) {
        return const Center(child: Text('No se encontraron ventas pendientes a cargar.'));
      }
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return InkWell(
            onTap: () {
              // Cuando se hace clic en un pedido, navega a la página de detalles.
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) => SaleDetailPage(
                    sale: sale,
                    saleDetails: sale.salesDetails, // Debes proporcionar la lista de detalles aquí.
                  ),
                ),
              );
            },
